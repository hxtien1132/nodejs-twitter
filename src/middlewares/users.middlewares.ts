    ...
    options: async (value: string, { req }) => {
      ...
      try {
        ...
        if (error instanceof JsonWebTokenError) {
          throw new ErrorWithStatus({
            message: capitalize(error.message), //token sai
            status: HTTP_STATUS.UNAUTHORIZED
          })
        }
        throw error
        ...
        custom: {
          options: async (value: string, { req }) => {
            ...
            try {
              ...
              throw new ErrorWithStatus({
                message: capitalize((error as JsonWebTokenError).message), //'Token sai':
                status: HTTP_STATUS.UNAUTHORIZED
              })
            }
...
export const veirifyForgotPasswordValidator = validate(
  checkSchema(
    {
