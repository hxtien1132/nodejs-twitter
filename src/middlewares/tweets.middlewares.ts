    ...
    medias: {
      ...
      custom: {
        options: (value, { req }) => {
          ...
          if (
            value.some((item: any) => {
              return typeof item.url !== 'string' || !mediaTypes.includes(item.type) //0,1,2
            })
          ) {
          ...
          options: async (value, { req, location, path }) => {
            ...
            if (!tweet) {
              throw new ErrorWithStatus({
                ...
                message: TWEETS_MESSAGES.TWEETS_NOT_FOUND
              })
            }
...
export const audienceValidator = wrapRequestHandler(async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (tweet.audience === TweetAudience.TwitterCircle) {
    ...
    // Kiểm tra người xem tweet này có trong Twitter Circle(có thể xem tweet) của tác giả hay không
